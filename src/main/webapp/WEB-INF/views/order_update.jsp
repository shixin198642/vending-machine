...
-->
	...
	<div class="mainbox">
		<div class="nav-panel clearfix">
			<div class="fl nav">
				<span>采购订单</span><i class="arrow arrow-gray-r"></i><span class="end">编辑采购订单</span>
			</div>
		</div>
							...
							<div class="bd">
								<div>
									<dl class="infobox">
											...
											<label for="" class="labelbox">
												<select class="drop-select drop-all" name="userid" id="userid" style="width: 270px;" onchange="getUserTelephone(this);">
													...
													<c:forEach items="${userList}" var="users">
														<option value="${users.id}" attr_tel="${users.mobile }" <c:if test="${order.userid == users.id}">selected="selected"</c:if>>${users.displayName}</option>
													</c:forEach>
												</select>
											</label>
										</dd>
										...
										<dd>
											<label for="" class="labelbox">
												<input type="text" id="usertel" name="usertel" value="${order.usertel}" />
											</label>
										</dd>
									</dl>
								</div>  
									...
									<dl class="infobox">
										...
										<dd>
											<label for="" class="labelbox" style="position: relative;">
												<input class="w386" type="text" id="suppliername" placeholder="供应商编号或公司名称" value="${order.suppliername}">
											</label>
										...
										<dd>
											<label for="" class="labelbox">
												<input class="w386 disabled-input" type="text" id="supplieraddress" name="supplieraddress" value="${order.supplieraddress}" disabled="disabled">
											</label>
										</dd>
										<dt><span class="star">*</span>法定代表人</dt>
										<dd>
											<label for="" class="labelbox">
												<input class="w386" type="text" id="supplierusername" name="supplierusername" value="${order.supplierusername}">
											</label>
										</dd>
										<dt><span class="star">*</span>电话</dt>
										<dd>
											<label for="" class="labelbox">
												<input class="w386" type="text" id="supplierusertel" name="supplierusertel" value="${order.supplierusertel}">
											</label>
										</dd>
										<dd style = "display:none">
											<input id="id" name="id"  value="${order.id}">
											<input id="orderstate" name="orderstate"  value="${order.orderstate}">
											<input id="supplierid" name="supplierid" value="${order.supplierid}">
											<input id="suppliercontactid" name="suppliercontactid" value="${order.suppliercontactid}" />
											<input id="supplieraccount" value="${order.payaccount}">
											<input id="supplierbank" value="${order.paybank}">
											<input id="payamt" name="payamt" value="${order.payamt}" />
										</dd>
									</dl>
								</div>
							</div>
				...
				<div class="info-panel">
					...
					<div class="bd">
							...
							<table width="100%" cellspacing="0" cellpadding="0">
								...
								<tbody class="ordersku">
									...
									<c:forEach items="${list}" var="orderSku" varStatus="status">
										<tr class="ordersku_row" id="osku${orderSku.id}">
											<td>${orderSku.skunumber}</td>
											<td>${orderSku.skuname}</td>
											...
											<td><span class="alink">
												<a class="colgreen" href="javascript:alterordersku('${orderSku.id}')" title="编辑">编辑</a>
												<a class="colgreen" href="javascript:deleteordersku('${orderSku.id}','${orderSku.amount}')" title="删除">删除</a>
											</span></td>
										</tr>
									</c:forEach>
									</c:if>	
						...
						<div class="clearfix bottom-info">
							...
							<div class="fr">
								<div class="total-sum">总计：<span id="totalamount">${order.payamt}</span></div>
							</div>
						</div>
					</div>
				</div>
					...
					<div class="bd">
						<dl>
							...
							<dd style="width:300px;">
								<span>
									<input type="radio" name="settlemode" class="input-radio" value="1" <c:if test="${order.settlemode == 1}">checked="checked"</c:if>/> 先款后货 
									<input type="radio" name="settlemode" class="input-radio" value="2" <c:if test="${order.settlemode == 2}">checked="checked"</c:if> style="margin-left:10px;"/> 先货后款
								</span>
							</dd>
						...
						<dl class="dl-style2">
							...
							<dd style="width:300px;">
								<label for="" class="labelbox">
									<input class="w166" type="text" name="payname" value="${order.payname}" />
								</label>
							</dd>
						...
						<dl class="dl-style2">
							...
							<dd style="width:300px;">
								<label for="" class="labelbox">
									<input class="w166" type="text" name="payaccount" value="${order.payaccount}" />
								</label>
							</dd>
							<dt style="width:70px;text-align: right;">开户行</dt>
							<dd>
								<label for="" class="labelbox">
									<input type="text" name="paybank" value="${order.paybank}" />
								</label>
							</dd>
						</dl>
					</div>
				...
				<div class="info-panel">
					...
					<div class="bd">
						<dl class="dl-style2">
								...
								<label for="" class="labelbox">
									<select class="drop-select drop-220" name="storehouseid" id="storehouseid" style="width: 270px;height: 34px;">
										...
										<c:forEach items="${storeList}" var="stores">
											<option value="${stores.id}" <c:if test="${order.storehouseid == stores.id}">selected="selected"</c:if>>${stores.name}</option>
										</c:forEach>
									</select>
								</label>
							</dd>
							...
							<dd>
								<label for="" class="labelbox">
									<input class="w200 disabled-input" type="text" id="storehouseaddress" name="storehouseaddress" value="${order.storehouseaddress}" disabled="disabled">
								</label>
							</dd>
						...
						<dl class="dl-style2">
							...
							<dd>
								<label for="" class="labelbox">
									<input type="text" id="storehousemanagername" name="storehousemanagername" value="${order.storehousemanagername}">
								</label>
							</dd>
							<dt><span class="star">*</span>联系电话</dt>
							<dd>
								<label for="" class="labelbox">
									<input type="text" id="storehousemanagertel" name="storehousemanagertel" value="${order.storehousemanagertel}">
								</label>
							</dd>
						</dl>
					</div>
				</div>
				<div class="fr btn-mod-panel"><input id="sure" type="checkbox" style="margin: 3px;">订单已编辑完毕，确认无误<a class="btn-mod btn-skin-green" href="javascript:send('alterordercontract')" title="订单已编辑完毕，确认无误，生成合同">完成，生成合同</a><a class="btn-mod btn-skin-gray" href="javascript:send('alterorder')" title="保存">保存</a><a class="btn-mod btn-skin-gray" href="<%=request.getContextPath()+WebPageConstants.COMMON_PREFIX+WebPageConstants.LIST_ORDER%>?menu_id=22" title="放弃编辑">放弃编辑</a></div>
			</div>
		</div>	
		</form>
	</div>
...
<script>
  ...
  function send(formname){
	  ...
	  if (formname == "alterordercontract") {
		  if ($("#sure")[0].checked) {
