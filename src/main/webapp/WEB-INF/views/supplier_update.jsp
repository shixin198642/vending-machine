...
-->
			...
			<div class="mainbox">
				<div class="nav-panel clearfix">
					<div class="fl nav">
						<span>供应商</span><i class="arrow arrow-gray-r"></i><span class="end">修改供应商</span>
					</div>
					<div class="fr btn-mod-panel"><a class="btn-mod btn-skin-gray" href="javascript:send('altersupplier')" title="保存">保存</a><a class="btn-mod btn-skin-gray" href="JavaScript:history.go(-1)" title="放弃编辑">放弃编辑</a></div>
				</div>
								...
								<dl class="infobox">
									...
									<dd>
										<label for="" class="labelbox">
											<input class="w386" type="text" id="sname" name="sname" value="${supplier.sname}" />
										</label>
									</dd>
									<dt><span class="star">*</span>通讯地址</dt>
									<dd>
										<label for="" class="labelbox">
											<input class="w500" type="text" name="address" value="${supplier.address}" />
										</label>
									</dd>
									<dt class="vtop">备注</dt>
									<dd>
										<textarea class="w520" name="remarks" id="" cols="30" rows="7">${supplier.remarks}</textarea>
									</dd>
									<dd style = "display:none">
										<input id="id" name="id" value="${supplier.id}">
									</dd>
								</dl>															
							</div>
						</div>
								...
								<div class="table table-d">
									<table width="100%" cellspacing="0" cellpadding="0">
										...
										<c:forEach items="${list}" var="supplierContact" varStatus="status">
											<tr class="suppliercontact" id="contact${supplierContact.id}">
												...
												<td><span class="alink"><a class="colgreen" href="javascript:altersuppliersc('${supplierContact.id}')" title="编辑">编辑</a><a class="colgreen" href="javascript:deletesuppliersc('${supplierContact.id}')" title="删除">删除</a></span></td>
											</tr>
										</c:forEach>
										</c:if>																			
										</tbody>
											...
											<dd>
												<label for="" class="labelbox">
													<select class="drop-select drop-all" name="mid" id="mid">
														...
														<c:if test="${list.size()>0}">
														<c:forEach items="${list}" var="supplierContact" varStatus="status">
															<option value="${supplierContact.id}" <c:if test="${supplierContact.type==1}">selected="selected"</c:if>>${supplierContact.cname}</option>
														</c:forEach>
														</c:if>	
													</select>
												</label>
					...
					<div class="info-panel">
						...
						<div class="bd">
							<dl class="dl-style2">
								...
								<dd>
									<label for="" class="labelbox">
										<input class="w150" type="text" name="payname" value="${supplier.payname}" />
									</label>
								</dd>
								<dt>银行账号</dt>
								<dd>
									<label for="" class="labelbox">
										<input class="w150" type="text" name="account" value="${supplier.account}" />
									</label>
								</dd>
							 </dl>
							 <dl class="dl-style2">
								...
								<dd>
									<label for="" class="labelbox">
										<input class="w150" type="text" name="bank" value="${supplier.bank}"/>
									</label>
								</dd>
								<dt>收货地址</dt>
								<dd>
									<label for="" class="labelbox">
										<input class="w386" type="text" name="shipaddress" value="${supplier.shipaddress}" />
									</label>
								</dd>
...
<script>
  ...
  function send(formname){
	  ...
	  if(!theform.valid()){
		...
		if(formname=="altersupplier" && $("#mid").val()=='-1'){
